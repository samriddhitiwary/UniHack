"""Opt-in DynamoDB Local contract test for extraction-result persistence."""
...
pytestmark = pytest.mark.skipif(
    ...
    reason="set RUN_DYNAMODB_INTEGRATION=1 after creating the extraction-results table",
)


def test_pdf_extraction_repository_contract_against_dynamodb_local() -> None:
    settings = get_settings()
    client = create_dynamodb_client(settings)
    table_name = settings.table_name("extraction-results")
    repository = DynamoDBPdfExtractionResultRepository(client, table_name)
